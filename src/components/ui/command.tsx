...
const Command = React.forwardRef<React.ElementRef<typeof CommandPrimitive>, React.ComponentPropsWithoutRef<typeof CommandPrimitive>>(({ className, ...props }, ref) => (
    <CommandPrimitive ref={ref} className={cn('flex h-full w-full flex-col overflow-hidden rounded-[20px] bg-popover text-popover-foreground', className)} {...props} />
));
    ...
    ({ className, ...props }, ref) => (
        <div className="flex w-full items-center border rounded-[20px] shadow-xl px-3" cmdk-input-wrapper="">
            ...
            <CommandPrimitive.Input
                ref={ref}
                className={cn('flex h-10 w-full rounded-md  py-3 text-sm outline-none placeholder:text-[#696F79] disabled:cursor-not-allowed disabled:opacity-50', className)}
                {...props}
            />
        </div>
    )
